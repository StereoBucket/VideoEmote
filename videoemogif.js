...
class Processor{
	...
	//Declare all class member variables used.
		...
		this.canvas_arr = [];
		this.canvas_arr_cx = [];
		this.canvas_width = 32; // TODO: Make user input-able.
		this.gif_info = {
			fps:29.97,		
			speed:1,	//Speed relative to the video. 1 is the same speed. 0.5 is 50% speed...
			quality:10,
			dispose:0,
		};
		this.gifs = [];
	...
	stepFrameProc()
	{
		...
		{
			this.gifs[0].finish();
			//this.gifs[0].download("gif.gif");
			document.getElementById("gifgif").src='data:image/gif;base64,'+encode64(this.gifs[0].stream().getData());
			return;
		}
		//if(this.video_input.currentTime >= this.upper_time_bound)
		//	return;
		let step = 1/parseInt(this.fps_input.value);
		this.videoCutter();
		this.addFrame();
		this.video_input.currentTime +=step;
		let self = this;
		setTimeout(function(){self.stepFrameProc();},0);
	}
	adjustCanvasGrid()
	{
	...
	startEncode()
	{
		...
		for(let i = 0; i < num ; i++)
		{
			let g = new GIFEncoder();
			g.setRepeat(0);
			g.setFrameRate(this.gif_info.fps);
			g.start();
			this.gifs.push(g);
		}
	...
	addFrame()
	{
		...
		{
			this.gifs[i].addFrame(this.canvas_arr_cx[i]);
		}
	}
	finishEncode()
	{
